  ...
  const arr = ccn
    ...
    .map((el, index) => {
      if (index % 2 === 1) {
        let newEl = el * 2;
        if (`${newEl}`.length > 1) {
          newEl = +`${newEl}`.split('').reduce((acc, value) => acc + +value, 0);
        }
        return newEl;
      }
      return +el;
    });
  return arr.reduce((acc, value) => acc + value, 0) % 10 === 0;
}
...
function getDigitalRoot(num) {
  let sum = `${num}`.split('').reduce((acc, value) => acc + +value, 0);
  if (`${sum}`.length > 1) {
...
function isBracketsBalanced(str) {
  ...
  const leftBracketArr = [];
  for (let i = 0; i < str.length; i += 1) {
    if (Object.keys(bracketKeys).includes(str[i])) {
      leftBracketArr.push(str[i]);
    } else if (
      bracketKeys[leftBracketArr[leftBracketArr.length - 1]] === str[i]
    ) {
      leftBracketArr.pop();
    } else {
      return false;
...
function getCommonDirectoryPath(pathes) {
  const arrSplitPathes = pathes.map((el) => el.split('/'));
  const commonArr = [];
  for (let i = 0; i < arrSplitPathes[0].length; i += 1) {
    if (arrSplitPathes.every((el) => el.includes(arrSplitPathes[0][i]))) {
      commonArr.push(arrSplitPathes[0][i]);
    }
  }
  return commonArr.length !== 0 ? `${commonArr.join('/')}/` : '';
}
