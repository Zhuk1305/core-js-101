...
function getPolynom(...args) {
  ...
  if (args.length === 2) return (a) => args[0] * a + args[1];
  const func = (a) => args[0] * a ** 2 + args[1] * a + args[2];
  return func;
}
...
function logger(func, logFunc) {
  return (...args) => {
    const funcArgs = JSON.stringify(args).slice(1, -1);
